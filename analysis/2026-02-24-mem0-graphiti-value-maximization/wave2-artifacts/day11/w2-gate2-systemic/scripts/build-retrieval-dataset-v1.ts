...
const datasetPath = path.join(inputsDir, "retrieval-dataset.v1.json");
const notesPath = path.join(evidenceDir, "dataset-notes.md");
const buildLogPath = path.join(inputsDir, "retrieval-dataset.v1.build.log");
const candidatesPath = path.join(inputsDir, "retrieval-dataset.v1.candidates.json");

const seeds: string[] = [
  // Routing / upgrade / bridge
  "mem0 graphiti bridge",
  "hybrid plan combines mem0 and graphiti",
  "记忆 机制 升级",
  "Mem0 Graphiti 记忆升级",
  "graphiti",
  "mem0",
  ...
  "weekly hygiene rule",
  "bootstrap-only mode",
  // Domain context (Jarvis)
  "Jarvis OS mission",
  "Jarvis thinktank",
  "Jarvis 操作系统",
  // Misc recall
  "founder priority Telegram",
  "Telegram 优先",
  "thinktank topology",
  "topology blueprint",
];
...
const main = async (): Promise<void> => {
  ...
  buildLog.push(`[dataset-v1] generated_at_utc=${new Date().toISOString()}`);
  buildLog.push(`[dataset-v1] graphiti_base_url=${graphitiBaseUrl}`);
  buildLog.push(`[dataset-v1] max_facts=${String(maxFacts)}`);
  buildLog.push(`[dataset-v1] timeout_ms=${String(timeoutMs)}`);
  buildLog.push(`[dataset-v1] target_size=${String(targetSize)}`);
  buildLog.push(`[dataset-v1] seeds_count=${String(seeds.length)}`);
  ...
  if (selected.length < 30) {
    ...
    throw new Error(`dataset-v1: selected ${selected.length} (<30)`);
  }
  ...
  const notes = [
    ...
    "",
    "```bash",
    `node --import tsx ${path.relative(process.cwd(), path.join(scriptDir, "build-retrieval-dataset-v1.ts"))}`,
    "```",
    "",
    "## Source",
    "",
