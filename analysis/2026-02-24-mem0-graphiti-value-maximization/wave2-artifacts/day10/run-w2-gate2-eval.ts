...
const runGate2 = async (): Promise<void> => {
  ...
  const runOneRetrievalEval = async (params: {
    ...
    let activeContext: { context: QueryContext; url: string; requestBody?: unknown } | null = null;
    ...
    const recordingFetch: typeof fetch = async (input, init) => {
      ...
      if (activeContext && url.includes("/search")) {
        ...
        await writeJsonFile(path.join(graphitiRunDir, `search.raw.${fileLabel}.json`), {
          ...
          requestBody: activeContext.requestBody ?? null,
          payload,
        });
      }
      return response;
    ...
    const client = {
      ...
      search: async (query: string) => {
        ...
        activeContext = { context, url: `${graphitiBaseUrl}/search`, requestBody };
  ...
  const report: Gate2Report = {
    ...
    changed_files: [
      "extensions/memory-mem0-graphiti-bridge/src/router/query-signature.ts",
  ...
  await writeText(
    path.join(gate2Dir, "w2-master-closure.md"),
    [
      "# Wave2 Master Closure (Single Source of Truth)",
      "",
      `- Final Gate-2 conclusion: **${report.gate2_conclusion}**`,
      `- Evidence path: \`analysis/2026-02-24-mem0-graphiti-value-maximization/wave2-artifacts/day10/w2-gate2-report.json\``,
      `- Review notes: \`analysis/2026-02-24-mem0-graphiti-value-maximization/wave2-artifacts/day10/w2-gate2-review.md\``,
      "",
      "Stop point: W2-D complete; wait for confirmation before any post-Gate actions.",
      "",
    ].join("\n"),
  );
