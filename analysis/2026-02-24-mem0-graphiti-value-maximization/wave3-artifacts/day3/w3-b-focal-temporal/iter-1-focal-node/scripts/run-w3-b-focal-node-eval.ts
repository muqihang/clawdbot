...
const ITER_ROOT =
  "analysis/2026-02-24-mem0-graphiti-value-maximization/wave3-artifacts/day3/w3-b-focal-temporal/iter-1-focal-node";
...
const buildMainPayload = (sample: TemporalDatasetSample, hasFocalNode: boolean): MockPayload => {
      ...
      facts: [
        {
          ...
          score: 0.9,
          fact: `focal facts context for ${sample.id}`,
        },
      ],
      episodes: [
        {
          id: expectedId,
          score: 0.97,
          summary: `focal episode expected for ${sample.id}`,
        },
      ],
      nodes: [
        {
          ...
          score: 0.89,
          name: `focal node context for ${sample.id}`,
        },
      ],
    };
  }
...
const createFlags = (mode: Mode): BridgeFlags => {
  const focalEnabled = mode === "variant";
  ...
  return {
    ...
    read: {
      ...
      graphiti_focal_node: {
        enabled: focalEnabled,
        sample_percent: focalEnabled ? 100 : 0,
      },
    },
    p3: {
    ...
    const graphitiClient = {
      search: async (query: string, options?: Record<string, unknown>) => {
        ...
        const payload = discoveryCall
          ...
          : buildMainPayload(sample, Boolean(focalNodeUuid));
