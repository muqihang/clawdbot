...
const ITER_ROOT =
  "analysis/2026-02-24-mem0-graphiti-value-maximization/wave3-artifacts/day3/w3-b-focal-temporal/iter-2-temporal-filters";
const DATASET_PATH = path.join(ITER_ROOT, "inputs/retrieval-temporal-dataset.v1.json");
...
const buildDiscoveryPayload = (sample: TemporalDatasetSample): MockPayload => {
  return {
    facts: [],
    episodes: [],
    nodes: [
      {
        id: `w3b-focal-${sample.id}`,
        score: 0.98,
        name: `focal node for ${sample.id}`,
      },
    ],
  };
};

const buildMainPayload = (params: {
  sample: TemporalDatasetSample;
  hasFocalNode: boolean;
  hasTemporalFilters: boolean;
}): MockPayload => {
  const { sample, hasFocalNode, hasTemporalFilters } = params;
  const expectedId = sample.expected_ids?.[0] ?? `missing-${sample.id}`;

  if (hasFocalNode && hasTemporalFilters) {
    return {
      facts: [
        {
          id: `w3b-fact-${sample.id}`,
          score: 0.9,
          fact: `temporal-filtered facts context for ${sample.id}`,
        },
      ],
      episodes: [
        {
          id: expectedId,
          score: 0.97,
          summary: `temporal-filtered episode expected for ${sample.id}`,
        },
      ],
      nodes: [
        {
          id: `w3b-node-${sample.id}`,
          score: 0.89,
          name: `temporal-filtered node context for ${sample.id}`,
        },
      ],
    };
  }
      ...
      facts: [
        {
          id: expectedId,
          score: 0.95,
          fact: `focal-only expected stays in facts for ${sample.id}`,
        },
      ],
      episodes: [
        {
          id: `w3b-episode-miss-${sample.id}`,
          score: 0.96,
          summary: `focal-only episode distractor for ${sample.id}`,
        },
      ],
      nodes: [
        {
          id: `w3b-node-${sample.id}`,
          score: 0.9,
          name: `focal-only node context for ${sample.id}`,
        },
      ],
    };
  }
    ...
    facts: [
      {
        id: expectedId,
        score: 0.96,
        fact: `baseline expected exists but not elevated for ${sample.id}`,
      },
      {
        id: `w3b-fact-alt-${sample.id}`,
        score: 0.92,
        fact: `baseline facts alternate for ${sample.id}`,
      },
    ],
    episodes: [
      {
        id: `w3b-episode-miss-${sample.id}`,
        score: 0.95,
        summary: `baseline episode distractor for ${sample.id}`,
      },
    ],
    nodes: [
      {
        id: `w3b-node-${sample.id}`,
        score: 0.88,
        name: `baseline node context for ${sample.id}`,
      },
    ],
  };
};
...
const createFlags = (mode: Mode): BridgeFlags => {
  const temporalFiltersEnabled = mode === "variant";
  ...
  return {
    ...
    read: {
      ...
      graphiti_temporal_filters: {
        enabled: temporalFiltersEnabled,
        sample_percent: temporalFiltersEnabled ? 100 : 0,
      },
    },
    p3: {
    ...
    const graphitiClient = {
      search: async (query: string, options?: Record<string, unknown>) => {
        ...
        const payload = discoveryCall
          ? buildDiscoveryPayload(sample)
          : buildMainPayload({
              sample,
              hasFocalNode: Boolean(focalNodeUuid),
              hasTemporalFilters: Boolean(temporalFilters),
            });
...
const main = async (): Promise<void> => {
  ...
  await writeJson(path.join(INPUTS_DIR, "runner-input.snapshot.json"), {
    ...
    mode: "W3-B-iter2",
    variable: "graphiti_temporal_filters",
    baseline_flags: {
      graphiti_recipe_routing: {
        enabled: true,
        sample_percent: 100,
  ...
  const toStabilityFieldValues = (runs: RunMetrics[], field: keyof RunMetrics): number[] => {
    return runs.map((item) => item[field] as number);
  };
