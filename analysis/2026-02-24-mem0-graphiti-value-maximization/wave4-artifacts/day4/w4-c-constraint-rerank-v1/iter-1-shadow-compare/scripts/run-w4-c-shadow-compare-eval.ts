...
const runToolOnce = async (params: {
  ...
  const mem0Search = async (): Promise<BridgeSearchHit[]> => [
    buildHit("mem0", "m-shadow", 0.81),
    buildHit("mem0", "shared", 0.8),
  ];
  const graphitiSearch = async (): Promise<BridgeSearchHit[]> => [
    buildHit("graphiti", "g-shadow", 0.8),
    buildHit("graphiti", "shared", 0.79),
  ];
