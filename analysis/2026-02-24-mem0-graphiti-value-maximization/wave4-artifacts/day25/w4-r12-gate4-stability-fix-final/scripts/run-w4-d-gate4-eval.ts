...
const main = async (): Promise<void> => {
  ...
  const runMode = async (mode: Mode, flags: unknown): Promise<ModeReport> => {
    ...
    const selectedBoundedSummary =
      boundedSummaries[selectedBoundedAttempt - 1] ?? boundedSummaries[0];
    ...
    const p50 = percentile(totalMs, 0.5);
    ...
    return {
      ...
      latency: {
        bounded_sample_count: totalMs.length,
  ...
  const baselineReport = await runMode("baseline", baselineFlags);
  const variantReport = await runMode("variant", variantFlags);
