...
export type ApprovalApiClient = {
  listApprovalTickets(
    params: ListApprovalTicketsParams,
  ): Promise<ApprovalApiResult<{ items: ApprovalTicketRecord[] }>>;
  getApprovalTicket(
    params: GetApprovalTicketParams,
  ): Promise<ApprovalApiResult<ApprovalTicketRecord>>;
...
export function createApprovalApiClient(
  options: CreateApprovalApiClientOptions,
): ApprovalApiClient {
      ...
      if (!response.ok) {
        return {
          ...
          error: {
            code: parseErrorCode(
              payload,
              response.status >= 500 ? "DEG_JARVIS_UNREACHABLE" : "APPROVAL_API_ERROR",
            ),
