...
export function createMemoryCommitGate(params: {
  ...
  handleCommitRequest(input: { proposalId: string; body: unknown }): Promise<MemoryCommitGateResult>;
} {
  ...
  return {
    async handleCommitRequest(input: { proposalId: string; body: unknown }): Promise<MemoryCommitGateResult> {
